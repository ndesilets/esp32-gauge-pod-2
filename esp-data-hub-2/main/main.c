...
// *slurp noises intensifies*
void data_slurper_task(void* arg) {
  // send ssm request for ecu data
  // send ssm request for abs data

  // wait for responses and update state
}
...
#else
  xTaskCreate(data_slurper_task, "data_slurper", 8192, NULL, tskIDLE_PRIORITY + 1, NULL);
