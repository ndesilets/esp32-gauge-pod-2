...
static esp_err_t real_init(void) {
  if (!s_i2c_initialized) {
    ...
    ESP_RETURN_ON_ERROR(i2c_master_bus_add_device(s_i2c_bus, &dev_cfg, &s_ads1115), TAG,
                        "i2c_master_bus_add_device failed");

    ESP_RETURN_ON_ERROR(i2c_master_probe(s_i2c_bus, CONFIG_DH_ANALOG_ADS1115_ADDR, 50), TAG,
                        "ADS1115 probe failed");
...
static esp_err_t real_read(analog_sensor_reading_t *out) {
  ...
  ESP_RETURN_ON_ERROR(ads1115_read_single_ended_raw(ADS1115_MUX_AIN2_GND, &raw_temp), TAG, "temp read failed");
  ESP_RETURN_ON_ERROR(ads1115_read_single_ended_raw(ADS1115_MUX_AIN3_GND, &raw_pressure), TAG,
