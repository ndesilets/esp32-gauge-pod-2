...
typedef struct {
  ...
  struct {
    unsigned int use_qspi_interface
        : 1; /*<! Set to 1 if use QSPI interface, default is SPI interface */
  } flags;
 ...
 *
 */
#define AXS15231B_PANEL_BUS_I80_CONFIG(dc, wr, clk, d0, d1, d2, d3, d4, d5, \
                                       d6, d7, b_w, max_trans_sz)           \
  {                                                                         \
      .dc_gpio_num = dc,                                                    \
      .wr_gpio_num = wr,                                                    \
      .clk_src = clk,                                                       \
 ...
 *      - ESP_OK: on success
 */
esp_err_t esp_lcd_touch_new_i2c_axs15231b(const esp_lcd_panel_io_handle_t io,
